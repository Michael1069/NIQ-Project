...
if (!gotTheLock) {
  console.log('----------------------------------------------------');
  console.log('[Main] NIQ HelpDeskAI is already running in System Tray!');
  console.log('[Main] Sidebar toggled via active background process.');
  console.log('----------------------------------------------------');
  app.quit();
} else {
...
if (gotTheLock) {
  app.whenReady().then(() => {
    ...
    // Register suite of global hotkeys on the primary process only
